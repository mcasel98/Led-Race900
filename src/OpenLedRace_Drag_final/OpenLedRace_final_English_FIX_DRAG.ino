 ...
 * ____                     _      ______ _____    _____
 ...
 ------------------------------------------------------------------------
 LED Race 900 [DRAG] - Major upgrades by Marcello Caselli (mcasel98) Jan 2026
 Full rewrite, game logic, and advanced features powered by GitHub Copilot
 Special thanks to OpenAI Copilot for assistance, code review and race-inspired ideas!

 Key improvements:
 - Welcome message
 - Pit box people animation before the race
 - Count down: Ready-Set-Go!
 - Race leader tracking
 - Laps counter, Final Lap alert, Winner declaration
 - Podium light show
 - Winner music
 - Automatic return of racing cars to the start before next race
 - Fully non-blocking game logic: "FINAL LAP!!" is always fully shown, race never pauses
 - [2026] DRAG EFFECT: leader (4 leds) and second (3 leds) with gradient tail, active above drag speed threshold

 Released under the same GPLv3 license.
 For the full release and source: https://github.com/mcasel98/Led-Race900
*/
...
// --- PATCH: FINAL LAP MESSAGE and STATUS MANAGEMENT ---
bool showFinalLapActive = false;     // true = the matrix is dedicated to the message
bool finalLapShownOnce = false;      // true = message was already displayed
...
// ===================
//    DRAW CARS WITH DRAG EFFECT
// ===================
void drawCars() {
  constexpr float dragSpeed = 2; // Speed threshold above which the drag effect is shown
  int order[NUM_CARS];
  ...
  for (int p = 0; p < NUM_CARS; ++p) {
    ...
    // --- DRAG EFFECT for Leader and Second only if above speed threshold ---
    if (p == NUM_CARS - 1 && cars[idx].speed > dragSpeed) { // Leader
      // Two drag leds, with increasing fade
      int drag1 = (tailpos > 0) ? tailpos - 1 : TRACK_PIXELS - 1;
      int drag2 = (drag1 > 0) ? drag1 - 1 : TRACK_PIXELS - 1;
      // Fade 1: less intense
      uint32_t colDrag1 = ((CAR_COLORS[idx] & 0xFCFCFC) >> 2); // ~1/4
      // Fade 2: even fainter
      uint32_t colDrag2 = ((CAR_COLORS[idx] & 0xF8F8F8) >> 3); // ~1/8
      track.setPixelColor(drag1, colDrag1);
      track.setPixelColor(drag2, colDrag2);
    }
    else if (p == NUM_CARS - 2 && cars[idx].speed > dragSpeed) { // 2nd
      int drag1 = (tailpos > 0) ? tailpos - 1 : TRACK_PIXELS - 1;
      uint32_t colDrag1 = ((CAR_COLORS[idx] & 0xFCFCFC) >> 2); // ~1/4
      track.setPixelColor(drag1, colDrag1);
    }
...
/*** RACE DISPLAY: "FINAL LAP!!" ALWAYS FULLY SHOWN, LOGIC NEVER BLOCKS ***/
void displayRaceEssentialMessage() {
  ...
  if (showFinalLapActive) {
    ...
    // Keep the matrix display dedicated to "FINAL LAP!!" until message scrolls off
...
void displayWinnerLoopMessage() {
  ...
  snprintf(win_msg, sizeof(win_msg), "   %s   WON   THE   RACE!      ", CAR_COLOR_NAMES[safeWinnerIdx]);
