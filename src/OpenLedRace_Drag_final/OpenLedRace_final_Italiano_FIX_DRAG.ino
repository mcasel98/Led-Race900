...
// ===================
//    DRAW CARS CON EFFETTO SCIA
// ===================
void drawCars() {
  constexpr float sciaSpeed = 2; // Velocità soglia sopra cui compare la scia
  int order[NUM_CARS];
  ...
  for (int p = 0; p < NUM_CARS; ++p) {
    ...
    // --- Effetto SCIA per Leader e Secondo SOLO se velocità sopra soglia ---
    if (p == NUM_CARS - 1 && cars[idx].speed > sciaSpeed) { // Primo in classifica
      // Due led di scia, sempre più fade
      int scia1 = (tailpos > 0) ? tailpos - 1 : TRACK_PIXELS - 1;
      int scia2 = (scia1 > 0) ? scia1 - 1 : TRACK_PIXELS - 1;
      // Fade 1: meno intenso
      uint32_t colScia1 = ((CAR_COLORS[idx] & 0xFCFCFC) >> 2); // ~1/4
      // Fade 2: debole
      uint32_t colScia2 = ((CAR_COLORS[idx] & 0xF8F8F8) >> 3); // ~1/8
      track.setPixelColor(scia1, colScia1);
      track.setPixelColor(scia2, colScia2);
    }
    else if (p == NUM_CARS - 2 && cars[idx].speed > sciaSpeed) { // Secondo in classifica
      int scia1 = (tailpos > 0) ? tailpos - 1 : TRACK_PIXELS - 1;
      uint32_t colScia1 = ((CAR_COLORS[idx] & 0xFCFCFC) >> 2); // ~1/4
      track.setPixelColor(scia1, colScia1);
    }
