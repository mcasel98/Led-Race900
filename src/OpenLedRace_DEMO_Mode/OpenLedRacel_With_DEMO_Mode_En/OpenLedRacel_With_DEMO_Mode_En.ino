 ...
 *  ____                     _      ______ _____    _____
   ...
   | |__| | |_) |  __/ | | | | |____| |____| |__| | | | \ \ (_| | (_|  __/
    \____/| .__/ \___|_| |_| |______|______|_____/  |_|  \_\__,_|\___\___|
          | |
          |_|
  Open LED Race 900 [DRAG] + INFINITE DEMO MODE + HARDWARE DEBOUNCED START BUTTON
  Created by: mcasel98 & Copilot - January 2026

  DEMO Mode: cars run faster (+100%), infinite loop, announcement, podium for 5.5s, always reset with START.
  START button with hardware debounce routine (physical and logical debounce):  
    - Set `constexpr float DEMO_ACCEL_FACTOR = 2.0f;` for a "super show" demo.
    - Hold START for 3s in menu and release to enter DEMO.  
    - Press START anytime in DEMO to return to the menu.  
    - No false start: hardware debounce prevents false detection due to switch bounce.
...
constexpr float DEMO_ACCEL_FACTOR= 2.0f; // +100% demo speed
...
// Debounce parameters (Start)
constexpr unsigned long DEBOUNCE_MS = 45; // Typical HW value: 20-60ms, robust here

constexpr const char* CAR_COLOR_NAMES[NUM_CARS] = {"RED", "GREEN", "BLUE", "WHITE", "YELLOW"};
...
// --- DEMO: podium timing & announcement ---
unsigned long demoPodioStart = 0;
unsigned long demoAnnounceStart = 0;
bool demoAnnounceShown = false;

// --- HARDWARE DEBOUNCE START ---
bool startBtnState = HIGH;                // Current read state
bool lastStartBtnStable = HIGH;           // Last stable state
unsigned long lastDebounceTime = 0;

// Read button with debounce: returns only "stable" transitions
...
// Returns true only when the button transitions from HIGH->LOW after debouncing
bool isStartPressedDebounced() {
  debounceUpdateStartBtn();
  return startBtnState == LOW;
}
// Returns true only when the button transitions from LOW->HIGH after debouncing (release)
...
void raceCountdown() {
  struct {
    ...
    {"   STEADY   ",      track.Color(255,255,0), 880},
...
void displayDemoAnnounce() {
  matrix.displayClear();
  matrix.displayText("   DEMO MODE - AUTOMATIC CAR PRESENTATION   ", PA_CENTER, 16, 0, PA_SCROLL_LEFT, PA_SCROLL_LEFT);
...
void loop() {
  ...
  case RaceState::WAITING:
    ...
    if (startBtnState == LOW && !startWasDown) { // just pressed
    ...
    if (startBtnState == HIGH && startWasDown) { // just released
      if(startHeldActive) {
        if(demoTriggerReady) {
          resetRace();
          for(int i=0; i<NUM_CARS; ++i) cars[i].distance = 0;   // --- MOD: start at start line
          matrix.displayClear();
          matrix.displayText(
            "   DEMO MODE STARTING!        ",
